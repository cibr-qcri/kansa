...
const getTrackingFunctions = (optionsRef) => {
  ...
  const track = (event, properties, identifyPath = 'properties') => {
    ...
    if (__DEV__) {
      ...
      if (logToConsole) {
        console.info(
          `[analytics] Event '${event}' fired with props ${JSON.stringify(properties)}.`
        );
      }
    } else {
      amplitude.logEvent(event, properties);
  ...
  const identify = (properties) => {
    if (properties) {
      if (__DEV__) {
        ...
        if (logToConsole) {
          console.log(`[analytics] Tracked user updated with props ${JSON.stringify(properties)}.`);
        }
      } else {
        amplitude.setUserProperties(properties);
      }
...
const AnalyticsProvider = ({
  ...
  const initializeTracker = useCallback(async () => {
    ...
    if (__DEV__) {
      if (optionsRef.current.devOptions.logToTracker) {
        ...
        if (optionsRef.current.devOptions.logToConsole) {
          console.info(`[analytics] Events are logged to console, not tracker.`);
        } else {
          console.info(
  ...
  return <AnalyticsContext.Provider value={optionsRef}>{children}</AnalyticsContext.Provider>;
};
